...
> **This is the `v17` line.** Everything below is the v17 branch's own copy. The product graph,
> the hooks, the pipeline and the PR template all match `v18/dev`; what differs is the CMS major
> this line targets, which matters when you reach for an API.
...
```json
{"issue":{"repo":"umbraco/Umbraco.Automate","number":112,"title":"Step-output bindings are not resolved case-insensitively after a persistence round-trip"},"line":"v17","port":{"line":"v18","commit":"…"}}
```
...
1. **Work out the branch name first**, because it is also the idempotency key:
   ...
   the issue title. Example: `v17/feature/issue-112-binding-case-insensitive-lookup`.
...
3. **Get the workspace.** Call `ops-workspace · prepare` with the branch. Let it root the branch
   on the line's integration branch — do not resolve `v17/dev` here by hand. Everything below
...
4. **Read before writing.** Read the issue in full, then `CLAUDE.md` at the repo root and the
   ...
   surrounding files. **This line targets Umbraco CMS 17.x** — check the root `CLAUDE.md` for the
   exact range before reaching for an API.
...
5. **Make the change.**
   ...
   - **A port** (`port` is non-null): start from `port.commit`, then **adapt it to the target
     line**. Cherry-picking blind is not a port. Most work reaches v17 as a backport from v18,
     and the CMS major differs between the two lines, so APIs and namespaces genuinely move. A
     conflict is a signal to think, not to force. If the change cannot be adapted, stop and
     return `{"ok": false, "detail": "…"}` naming what diverged.
...
8. Return `{"ok": true, "branch": "…", "summary": "…"}`. Write the summary as PR-body prose: what
   changed, why, and how a reviewer can test it.
   ...
   **Leave the port decision open, and say so.** End the summary with the template's *Other
   ...
   When this change **is itself** a backport (`port` was non-null), say which line and PR it came
   from and title the PR the way the repo already does — `… (v17 backport)`, as in #175 and #176.
...
```json
{"branch":"v17/feature/issue-112-binding-case-insensitive-lookup"}
```
...
```json
{"landed":{"repo":"umbraco/Umbraco.Automate","pr_number":175,"line":"v17"}}
```
...
2. **Work out the target lines from the human's confirmed port decision** — never from
   ...
   | A line-scoped label on the issue (e.g. `v18`) | targets = the landed line plus that line |
